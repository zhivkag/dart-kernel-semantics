...
\[
  \begin{array}{ccll}
    ...
    \typemeta & : & \dtype & \text{domain of Dart types}\\
...
\newcommand{\StaticGet}[1]{\synt{\{}#1\synt{\}}}
    ...
    \begin{align}
        &\begin{multlined}
            \cesktranswheresplit%
                ...
                {\text{where $\membermeta$ is a static field and $\membermeta \in \dom(\env^{M}$)}}
            \end{multlined}
...
\begin{figure}[Htp]
    ...
    \begin{align}
        ...
        \cesktranswheresplit%
            ...
            {\parbox{10cm}{where:\\
                \(\env' = \{(\this,\,\env(\this))\}\),
                \(\deref{\env(\this)} = ((\textit{c}, \_, \_), \_)\),
                \(c = (\_, \_, \textit{mems})\),
                \(\textit{mems}(\idmeta) = \text{Method}(\formals, \stmt)\)}}
        \end{multlined}
        \label{eval:superpropertyget-tearoff}\\
        &\begin{multlined}
        \cesktranswheresplit*%
            {\evalconf{\SuperPropertyGet{\idmeta}}{\env}{\strace}{\cstrace}{\cex}{\econt}}%
            {\execconf{\stmt}{\env'}{[]}{[]}{\SuperPropertyGet{\idmeta} :: \strace}{\handler}{\cex,\,\cstrace}{\econt}{\scont}}%
            {\parbox{10cm}{where:\\
                \(\deref{\env(\this)} = ((c, \_, \_), \_)\),
                \(c = (\_, \_, \textit{mems})\),
                \(\textit{mems}(\idmeta) = \text{Getter}(S)\),
                \(\env' = \{(\this, \env(\this))\}\)
                }}
        \end{multlined}
        ...
        \cesktranswheresplit%
            {\evalconf{\SuperPropertyGet{\idmeta}}{\env}{\strace}{\cstrace}{\cex}{\econt}}%
            {\contconf{\econt}{\deref{\textit{fields}[i]}}}%
            {\parbox{10cm}{where:\\
                \(\deref{\env(\this)} = ((c, \_, \_), \textit{fields})\),
                \(c = (\_, \_, \textit{mems})\),\\
                \(\textit{mems}(\idmeta) = \text{Getter}(i \in \mathbb{N})\)
            }}
        \end{multlined}
...
\begin{figure}[Htp]
    ...
    \cesktranswheresplit*%
        ...
        {\parbox{10cm}{where:\\
            \(v = (c, \_)\),
            \(c = (\_, \_, \textit{mems})\),
            \(\textit{mems}(X) = \text{Method}(\formals, \stmt)\),\\
            \(\env = \{(\this, \loc)\}\),
            \(\loc\) is fresh against the store,
            \(\deref{\loc} = \val\)
        }}
    \end{multlined}
    ...
    &\begin{multlined}
        \cesktranswheresplit*%
        {\contconf{\PropertyGetK}{\val}}%
        {\execconf{\stmt}{\env}{[]}{[]}{\PropertyGet{\val}{\idmeta} :: \strace}{\handler}{\cex,\,\cstrace}{\econt}{\emptyset}}%
        {\parbox{10cm}{where:\\
            \(\val = (c, \_)\),
            \(c = (\_, \_, \textit{mems})\),
            \(\textit{mems}(X) = \text{Getter}(\stmt)\),\\
            \(\env = \{(\this, \loc)\}\),
            \(\loc\) is fresh against the store,
            \(\deref{\loc} = \val\)
        }}
    ...
    &\begin{multlined}
        ...
        {\parbox{10cm}{where:\\
            \(v = (c, \textit{fields})\),
            \(c = (\_, \_, \textit{mems})\),
            \(\textit{mems}(X) = \text{Getter}(i \in \mathbb{N})\)
        }}
    \end{multlined}
    ...
    &\begin{multlined}
        ...
        {\contconf{\econt}{\funval{\formals}{\stmt}{\env}}}%
        {\parbox{10cm}{where $\membermeta = \text{Method}(\formals, \stmt)$ and $\env = \ext{\env_{empty}}{\mathbf{this}}{\val}$}}
    \end{multlined}
    \label{econtconf:dpropertyget-tearoff}\\
    &\begin{multlined}
        \cesktranswheresplit*%
            {\contconf{\DPropertyGetK}{\val}}%
            {\execconf{\stmt}{\env}{[]}{[]}{\DirectPropertyGet{\val}{\membermeta} :: \strace}{\handler}{\cex,\,\cstrace}{\econt}{\emptyset}}%
            {\parbox{10cm}{where $\membermeta = \text{Getter}(\stmt)$, $\env = \ext{\env_{empty}}{\mathbf{this}}{\val}$}}
    \end{multlined}
    ...
    &\begin{multlined}
        ...
        {\contconf{\econt}{\deref{\textit{fields}[i]}}}%
        {\parbox{10cm}{where
            \(\val = (\_, \textit{fields})\), \(\membermeta = \text{Getter}(i \in \mathbb{N})\)}}
    \end{multlined}
...
\subsection{Yield}
\todo{dmitryas: Write this section after native yield behavior is figured out, assuming that this section is about native "yield."}
