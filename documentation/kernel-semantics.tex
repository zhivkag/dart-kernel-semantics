...
\noindent
After the evaluation of the initializer expressions, the labels for each field in the object value are updated accordingly. An update of field $f_i$, stored at location $\loc' = (\deref\loc)[f_i]$ with value $\synval_i$ is expressed as $\deref\loc' = \deref\deref\loc[f_i] = \synval_{i}$.

\noindent
Let $Fs$ be the list of fields for value $\deref\loc$ for the class immediately enclosing the current constructor below.

If the initializer list, $Izs$ is empty,
\[
  \begin{array}{lcl@{\hspace{30mm}}}
	\acontconf{\fieldsapp{\env}{Izs}{\loc}{\scont}}{\synval{s}}
	& \Rightarrow &
	\scontconf{\scont}{\env} \textrm{, } Izs = []\\[1mm]
	\textrm{with }\deref\deref\loc[f_i] = \synval_{i}, \forall f_i \in Fs, \forall \synval_i \in \synval{s}
  \end{array}
\]

The initializer list of non-redirecting generative constructor is executed next.
%<InstanceFields(Super(args, Q(Sbody, is))), ρ, α, KS), vs>acont ->
%<args, ρ, SuperA(Sbody, is, α, KS)>evalList  	where  !α[fs -> vs] after transition
%<InstanceFields(I(E) :: is, ρ, α, KS), vs>acont ->
%<E, ρ, InitializerK(I, is, ↓ρ, α, KS)>eval, where !α[fs → vs] after transition
...
%<InitializerK(L(x), [], ρ, α, KS), V>cont → <Ks, ↑ρ>scont
%<InitializerK(L(x), I(E) :: is, ρ, α, KS), V>cont →
%<E,  ρ[x → α’],  InitializerK(I, is, ρ, α, KS)>eval where α’ s.t. !α’ = V
%<InitializerK(L(x), Super(args, Q(Sbody, is))) :: [], ρ, α, KS), V>cont →
%<args,  ρ[x → α’], SuperA(Sbody, is, α, KS) >evalList 	where α’ s.t. !α’ = V
%<InitializerK(L(x), Redirecting(args, Q) :: [], ρ, α, KS), V>cont →
%<args, ρ[x → α’], RedirectingA(G, α, KS)>evalList  	where α’ s.t. !α’ = V

Field initializers initialize of the fresh instance, i.e. they modify the value stored at location α in the store S, where !α = V, previously allocated fresh instance.

%<InitializerK(F(f), [], ρ, α, KS), V>cont → <Ks, ↑ρ>scont   	with !α[f → α’] and α’ s.t. !α’ = V	%<InitializerK(F(f), I(E) :: is, ρ, α, KS), V>cont →
%<E,  ρ,  InitializerK(I, is, ρ, α, KS)>eval  with !α[f → α’] and α’ s.t. !α’ = V
%<InitializerK(F(f), Super(args, Q(Sbody, is))) :: [], ρ, α, KS), V>cont →
%<args,  ρ, SuperA(Sbody, is, α, KS) >evalList 		with !α[f → α’] and α’ s.t. !α’ = V
%<InitializerK(F(f), Redirecting(args, Q) :: [], ρ, α, KS), V>cont →
%<args, ρ, RedirectingA(G, α, KS)>evalList  		with !α[f → α’] and α’ s.t. !α’ = V
\end{document}
