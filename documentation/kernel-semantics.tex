...
%<break L, ρ, Ls, Lswitch, H, st, cex, cst, KErr, KE, KS>exec → <K’B>break	with Label(L’, K’B, Ls’) ∈ Ls s.t.  L’ == L
%<Break(ρ, KS)>break → <KS, ρ>scont
%<FinallyBreak(S1, ρ, Ls, Lswitch, H, st, cex, cst, KE, KB)>break → 
%<S1, ρ, Ls, Lswitch, H, st, cex, cst, KE, BreakSK(KB)>exec
%<BreakSK(KB), _>scont → <KB>break
\subsection{Continue}

%<continue C, ρ, L,  Lswitch,  H, st, cex, cst, KE, KS>exec → <K’>switch  	
%with ContinueL(C’, K’, Ls’) ∈ Lswitch s.t.  C’ == C 

%<ContinueSwitch(S, ρ, Ls, Lswitch, H, st, KE, KS)>switch  → <S, ρ, Ls, Lswitch, H, st, KE, KS>exec
%<FinallyContinue(S, ρ, Ls, H, st, KE, Kswitch)>switch → <S, ρ, Ls, H, st, KE, SwitchSK(Kswitch)>exec
%<SwitchSK(K), _>exec → <K>switch
\subsection{Closures}
...
%< var x = f, ρ, Ls, Lswitch, H, st, cex, cst, KE, KS>exec → 
%<KS, ρ[x → α’]>scont 	where !α’ = FValue(f, α), !α = ρ[x → α’]
