...
A constructor has an associated class definition, an initializer list, $Izs$, with various kinds of initializers and a function node with formal parameters, $\formals$, and a statement body $\stmt$.

\kernel{} supports the following kinds of initializers:
...
Redirecting initializers appear last in the list of initializers of a constructor and we say the current constructor is a generative redirecting constructor.
The function node of a redirecting constructors does not have a statement body.
...
This ensures that side effects from evaluating the actual arguments for the target constructor invocation of the redirecting constructor occur before any side effects from initializing the fields in the immediately enclosing class.
...
Before invoking the target constructor of the super initializer, all non initialized fields in the immediately enclosing class are set to $\NullLiteral$.
\end{itemize}
...
The invocation of a constructor $Q$ proceeds by evaluating the argument expressions as shown in \eqref{eval:new}.
