...
\begin{document}
\title{Operational Semantics of Dart Kernel}
\author{Dart Kernel Team \\ \href{mailto:dart-kernel@google.com}{dart-kernel@google.com}}

\maketitle

\begin{abstract}

The \textsc{Dart Kernel} is a simplified representation of the \textsc{Dart} programming language.
Similar to the \textsc{Core} of the Glasgow Haskell Compiler, \textsc{Dart Kernel} is more amenable to formal specification and analysis than the \textsc{Dart} surface language and is the primary intermediate representation used in the \textsc{Dart} compilation toolchain.
This paper documents the syntax and operational semantics of \textsc{Dart Kernel} to enable framework authors for \textsc{Dart} to write safe transformations on \textsc{Dart Kernel}, to allow the creation of a reference interpreter for testing the backend implementations of \textsc{Dart} and to facilitate academic analysis of the safety properties of \textsc{Dart}.
...
Still, this paper aims to be as accessible as possible, so most concepts that are not familiar to an advanced \textsc{Dart} programmer will be explained in sufficient detail to understand the formalizations here.
...
It is instead a description of the data structures used by the \textsc{Dart} compliation toolchain, which are represented as directed acyclic graph.
...
The state space of the CESK machine is grouped into several configurations, most of which carry an environment, which maps variable declarations in the present scope to their current assignments, a store, which models the mutable object store in which all objects in a \textsc{Dart} program live, and a continuation, which captures the work to be done after the current operation is complete, roughly similar to a chain of call frames above the current function.
In this way states of the CESK machine mimic states of the \textsc{Dart} virtual machine at a very abstract level.
...
\[
  \begin{array}{ccll}
    ...
    \vals & : & \mlist{\dval} \\
    \vvec & : & \dvector = \mlist{\dval}& \text{domain of vectors}\\
    \env
    & : & \denv & \text{domain of environments}\\
    \obj
    & : & \dobjval & \text{domain of object values}\\
    \lit
    & : & \dlit & \text{domain of literal values}\\
    \classvar
    & : & \dclass & \text{domain of class definitions}\\
    \membermeta
    & : & \dmember & \text{domain of class members}\\
    && \dident & \text{domain of identifiers}
  \end{array}
\]
...
Values in \textsc{Dart Kernel} fall into two categories: $\dvector$ values (Section~\ref{subsubsec:vector-values}) and $\dobjval$ values (Section~\ref{subsec:object-values}).
...
\textsc{Dart Kernel} has limited support for arrays without bounds checking via $\dvector$s.
The step function for the CESK machine is not defined for operations that access an element vector outside its bounds.
$\dvector$s cannot be introduced by any construct present in \textsc{Dart} itself, but rather are introduced only by \textsc{Dart Kernel} transformations which can ensure they are only used in a safe manner.
...
A $\dclass$ encodes a class definition in Dart Kernel.

\begin{align*}
\dclass &= (\emptyset + \dclass) \times \mlist{\dclass} \times (\dident \mapsto \dmember)\\
\dmember &= \mathbf{Getter} + \mathbf{Setter} + \mathbf{Method} \\
\mathbf{Getter} &= \emptyset + \dstmt \\
\mathbf{Setter} &= \emptyset + \dformals \times \dstmt \\
\mathbf{Method} &= \dformals \times \dstmt
\end{align*}

A class definition contains a reference to the super class (none only in the case of the ``Object'' class), a list of interface classes and a finite map from identifiers to members which provides dynamic dispatch.
...
\todo{Specify what *valid* means and what error is thrown when arguments are not *valid*}
