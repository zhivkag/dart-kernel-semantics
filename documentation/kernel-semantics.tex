...
\label{subsec:literal-values}
There are multiple kinds of $\tt{literal}$ values:
\[
  \begin{array}{lcl}
...
\begin{align*}
  \getter \in \dobjval \times \mathbf{Name} &\rightarrow \dval \hspace{100mm} \\
  \getter(\val_0, fName) &= \val_1
\end{align*}
where $\val_1 = \deref{\loc}$.\\
...
\begin{align*}
  \setter \in \dobjval \times \mathbf{Name} \times \dval &\rightarrow \dval \hspace{100mm} \\
...
\subsection{Class}
\label{subsec:class}
A $\tt{class} \in \dclass$ component captures the different properties for a defined class in Kernel.
\[
\dclass : \tt{superclass} \times \tt{interfaces} \times \tt{fields} \times \tt{getters} \times \tt{setters} \times \tt{methods}
\]
...
The class component captures the immediate superclass and we allow access to it in the different continuations during their application in various steps defined below.
