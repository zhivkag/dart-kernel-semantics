...
This paper is targeted toward compiler authors, framework authors and academics.
The reader should have a proficient understanding of basic concepts in programming language design and implementation, such abstract syntax, intermediate representations and operational semantics.
Still, this paper aims to be as accessible as possible, so most concepts that are not familiar to an average \textsc{Dart} programmer will be explained in sufficient detail to understand the formalizations here.

\subsection{Dart Kernel}
The small-step operational
semantics of Dart Kernel is given by an abstract machine in the style of the
CESK machine. % TODO(dmitryas): Put \cite{cesk} here when we have bibliography.
The machine is defined by a single step transition function where each step of
the machine starts in a configuration and deterministically gives the next
configuration.
...
% todo: add async marker, type parameters, return type to FunctionValue
\[
...
\begin{array}{rl}
  ...
  % todo: Add sync*/async/async* marker and type parameters
...
% todo: Specify what *valid* means and what error is thrown when arguments are not *valid*
