...
%          WHERE
%
% "Split" variants has embedded "\\" inside.  Starred "split" variant have one
% after the comma.  Non-starred "split" variant have one additional "\\" after
...
\begin{figure}[Htp]
	...
	\end{align}
	\caption{The CESK-transition function for ValuePassingConfiguration: expressions}
...
Assigning a value to a variable $\varmeta$ will modify the store, more specifically the value stored at location  $\env(\varmeta)$. The evaluation of this expression proceeds by evaluating the right-hand side expression, \eqref{eval:varset} and setting the location $\env(\varmeta)$ in the store to the value this expression evaluates to, \eqref{econtconf:varset}.
...
%%
% Figure showing the CESK-transition function fstrating from EvalConfgiration for
...
\begin{figure}[Htp]
    \begin{align}
        ...
        \end{multlined}
    \end{align}
	\caption{The CESK-transition function for EvalConfiguration: instance property extraction and assignment expressions}
...
Let $\env^{M}$ be the global environment that stores bindings for static and library fields to locations for all static and library fields previously accessed or set during the execution of the program.\\
